...
func initialize(ctx context.Context) (context.Context, func(error)) {
	return ctx, func(err error) {
		...
		fmt.Fprintf(&b, cmdlog.ColorCyan(fmt.Sprintf(`You're running the community build of Atlas, which may differ from the official version.
...
`, release)))
		if err := cmdlog.WarnOnce(os.Stderr, b.String()); err == nil {
